...
#
#           test=1 BINDIR=/usr/local/bin CONFDIR=/etc/ddns-updater-dynu ./install.sh
...
Run cp --verbose bin/ddns-updater-dynu.sh $BINDIR/ddns-updater-dynu
Run chmod 755 $BINDIR/ddns-updater

file=ddns-updater-dynu.conf
CONFDIR=${CONFDIR:-$HOME/.config/ddns-updater-dynu}
...
case "$BINDIR" in
    /usr/local/bin* | /usr/bin*)
        Run cp cron.d/ddns-updater-dynu /etc/cron.d/
        ;;
    *)
        echo "\
